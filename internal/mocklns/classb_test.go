...
func TestClassBPingSlotDownlink(t *testing.T) {
	...
	lnsSrv := mocklns.New(mocklns.Config{
		...
		BeaconTime: 100000, Logger: quiet,
	})
	...
	tagRow, _ := st.Tags().Create(ctx, store.NewTag{
		...
		AppKey: hex.EncodeToString(appKey[:]), Class: "B", Region: "EU868",
	...
	want := []byte("ping-slot")
	time.Sleep(50 * time.Millisecond) // let hello registration land
	if err := lnsSrv.PushClassB(ctx, "0101010101010101", 10, want, 3); err != nil {
		t.Fatalf("PushClassB: %v", err)
	}

	select {
	case dl := <-gotDown:
		if !bytes.Equal(dl.Payload, want) {
			t.Errorf("Class B payload = %q, want %q", dl.Payload, want)
		}
	case <-time.After(3 * time.Second):
		t.Fatal("tag did not receive the Class B ping-slot downlink")
	}
}
